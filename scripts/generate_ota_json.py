...
def main():
    parser = argparse.ArgumentParser(
        ...
        usage='%(prog)s <文件1> <文件2> [选项]'
    )
    parser.add_argument('file1', help='Zip 文件或 build.prop 文件')
    parser.add_argument('file2', help='Zip 文件或 build.prop 文件')
    parser.add_argument('--version', help='版本标识符 (如 avium16)，若不提供则从文件名提取')
    parser.add_argument('--os', help='OS/ROM 名称 (如 AviumUI)，若不提供则从文件名提取')
    parser.add_argument('--base-url', default=DEFAULT_BASE_URL, dest='base_url',
                        help=f'下载基础 URL (默认: {DEFAULT_BASE_URL})')
    parser.add_argument('--device', help='设备代号 (如 lemonades)，若不提供则从文件名提取')
    parser.add_argument('--date', help='日期 (格式: YYYY-MM-DD，如 2026-02-26)，若不提供则从文件名提取')
    ...
    if args.date:
        ...
        print("错误: 在文件名中未找到日期 (YYYYMMDD)，且未通过 --date 参数提供。")
        sys.exit(1)

    # 设备代号: 优先使用 --device 参数，否则从文件名提取
    if args.device:
        device_codename = args.device
    elif date_index != -1:
        device_codename = parts[date_index - 1]
    else:
        print("错误: 无法确定设备代号，请通过 --device 参数提供。")
        sys.exit(1)

    # ROM 名称: 优先使用 --os 参数，否则从文件名提取
    if args.os:
        rom_name = args.os
    elif date_index != -1:
        rom_name = parts[0]
    else:
        print("错误: 无法确定 OS 名称，请通过 --os 参数提供。")
        sys.exit(1)

    # 从文件名提取版本号 (日期前第二项, 如 "16")
    rom_ver = parts[1] if date_index != -1 and len(parts) > 1 else ""
    ...
    # URL 版本部分: 优先使用 --version 参数，否则从 rom_name_normalized + rom_ver 构建
    if args.version:
        url_ver_part = args.version
        # 尝试从 --version 参数中提取版本号 (如 avium16 -> 16)
        ver_match = re.match(r'^[a-zA-Z]+(\d+)$', args.version)
        if ver_match:
            rom_ver = ver_match.group(1)
    else:
        url_ver_part = f"{rom_name_normalized}{rom_ver}"

    # 版本字符串 (用于 JSON 的 version 字段): 日期之前的所有部分
    if date_index != -1 and not args.os and not args.version:
        version_str = "-".join(parts[:date_index])
    else:
        version_str = f"{rom_name}-{rom_ver}" if rom_ver else rom_name
    ...
    path_ver_part = f"{rom_name_normalized}-{rom_ver}" if rom_ver else rom_name_normalized
